...
def saju_api():
    try:
        ...
        req = urllib.request.Request(url, data=json.dumps(gbody).encode(), headers={'Content-Type': 'application/json'}, method='POST')
        ...
        # 지역 제한 에러(Location not supported)가 나면 알려줌
        if "location" in err_body.lower():
            return jsonify({'error': {'message': '현재 서버 위치에서 구글 API를 호출할 수 없습니다. Render의 Region을 Oregon(미국)으로 변경해 보세요.'}}), 400
